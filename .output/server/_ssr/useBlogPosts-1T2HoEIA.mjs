...
//#region node_modules/.nitro/vite/services/ssr/assets/useBlogPosts-1T2HoEIA.js
function useBlogPosts(options = {}) {
	...
	return useQuery({
        ...
        `).order("created_at", { ascending: false }).range(offset, offset + limit - 1);
			...
			if (categoryId && data) {
				const { data: postCategories } = await supabase.from("blog_post_categories").select("post_id").eq("category_id", categoryId);
				const postIds = postCategories?.map((pc) => pc.post_id) || [];
				return data.filter((post) => postIds.includes(post.id));
			}
			return data;
		}
	});
}
function useBlogPost(slugOrId) {
	return useQuery({
		...
		queryFn: async () => {
          ...
          *,
          ...
          featured_image:blog_media!blog_posts_featured_image_id_fkey(id, url, alt_text)
        `);
			if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(slugOrId)) query = query.eq("id", slugOrId);
			else query = query.eq("slug", slugOrId);
			const { data, error } = await query.single();
			if (error) throw error;
			const [categoriesResult, tagsResult] = await Promise.all([supabase.from("blog_post_categories").select("category_id, blog_categories(*)").eq("post_id", data.id), supabase.from("blog_post_tags").select("tag_id, blog_tags(*)").eq("post_id", data.id)]);
			return {
				...data,
...
function useCreateBlogPost() {
	...
	return useMutation({
		mutationFn: async (data) => {
			...
			if (error) throw error;
			if (category_ids?.length) await supabase.from("blog_post_categories").insert(category_ids.map((id) => ({
				post_id: post.id,
				category_id: id
			})));
			if (tag_ids?.length) await supabase.from("blog_post_tags").insert(tag_ids.map((id) => ({
				post_id: post.id,
				tag_id: id
			})));
			return post;
	...
	return useMutation({
		mutationFn: async ({ id, data }) => {
			...
			if (category_ids !== void 0) {
				...
				if (category_ids.length) await supabase.from("blog_post_categories").insert(category_ids.map((catId) => ({
					post_id: id,
					category_id: catId
				})));
			}
			if (tag_ids !== void 0) {
				...
				if (tag_ids.length) await supabase.from("blog_post_tags").insert(tag_ids.map((tagId) => ({
					post_id: id,
					tag_id: tagId
				})));
			}
...
//#endregion
export { useUpdateBlogPost as a, useDeleteBlogPost as i, useBlogPosts as n, useCreateBlogPost as r, useBlogPost as t };
