...
- [ ] Done — commit `fix: populate TeamSettings form when team loads`
...
- [ ] Done — commit `fix: snapshot current content before version restore`
...
- [ ] Done — commit `fix: remove fake versioning UI from skill/workflow editors`
...
- [ ] Done — commit `fix: guard in-app navigation against unsaved editor changes`
...
**Verify:** build+lint.
- [ ] Done — commit `fix: run moderation on refine-update and apply-suggestion paths`
...
- [ ] Done — commit `fix: stop per-apply version inserts from AI coach`
...
**Verify:** build+lint.
- [ ] Done — commit `fix: surface v1 version insert failure on prompt create`
...
- [ ] Done — commit `feat: first-run empty state in Library`
...
- [ ] Done — commit `refactor: redirect /dashboard to /library, remove duplicate page`
...
- [ ] Done — commit `chore: remove dead Lovable-generated route island`
...
**Verify:** build+lint.
- [ ] Done — commit `fix: mobile Get Started opens signup tab`
...
- [ ] Done — commit `fix: remove claw 404 dead-ends from web UI`
...
- [ ] Done — commit `feat: tag deep-links filter Discover`
...
- [ ] Done — commit `fix: add site chrome to 404 page`
...
**Verify:** n/a (docs).
- [ ] Done — commit `docs: correct artifact-type claims in CLAUDE.md`
...
- [ ] Done — commit `feat: surface premium features to free users via UpsellModal (contact-support only)`
...
**Verify:** build+lint.
- [ ] Done — commit `feat: informative premium panel in Library footer`
...
- [ ] Done — commit `feat: surface AI credit balance in header and AI actions`
...
- [ ] Done — commit `refactor: single premium source of truth, drop 60s subscription polling`
...
- [ ] Done — commit `feat: SEO metadata on skill/workflow detail pages`
...
**Verify:** build+lint.
- [ ] Done — commit `fix: refetch instead of reload after Menerio sync`
...
- [ ] Done — commit `feat: send-to-LLM (and refine where supported) for skills/workflows`
...
- [ ] Done — commit `refactor: full VersionHistoryPanel in prompt editor`
...
- [ ] Done — commit `feat: unified team GitHub sync UI with explicit sync action`
...
**Verify:** build+lint.
- [ ] Done / SKIPPED (record which) — commit `feat: slug-redirect parity for skills/workflows/kits`
...
- [ ] Done — commit `perf: card-field selects + paginated discovery lists`
...
**Verify:** build+lint.
- [ ] Done — commit `refactor: shared useArtifactList factory`
...
- [ ] Done — commit `fix(security): authenticate notify-admin and escape email HTML`
...
- [ ] Done — commit `fix(security): credit-gate generate-embedding`
...
- [ ] Done — commit `fix(security): use is_admin RPC in delete-user and ensure-token-allowance`
...
- [ ] Done — commit `fix(security): origin allowlist on paid AI endpoints`
...
- [ ] Done — commit `fix(security): fail-closed credits check with clear infra-error message`
...
- [ ] Done — commits `refactor: config-driven <family> across artifact types` (4 commits: reviews+modal, copy-to-team, clone, suggest-metadata)

### Task G2 (requires DB access — attempt, else via Lovable agent): Real versioning for skills & workflows
...
- [ ] Done — commit `feat: search/sort/category parity across Discover tabs`

## Explicitly skipped (decision record)

- **Stripe checkout / customer portal / pricing page** — intentionally disabled (employment-contract compliance). Never restore.
- **Notifications settings** — remains "Coming soon" placeholder; harmless.
- **List virtualization** — re-evaluate only if E1 pagination proves insufficient.
