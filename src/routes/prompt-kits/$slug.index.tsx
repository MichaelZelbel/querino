...
import { loadPromptKit } from "@/lib/route-loaders";
...
function RouteComponent() {
  ...
  // record and never resets loading, so a client-side move from one kit to
  // another showed the old content under the new URL until the fetch resolved.
  const { slug } = Route.useParams();
  return <PromptKitDetail key={slug} initialKit={Route.useLoaderData()} />;
}
