...
export function CopyWorkflowToTeamModal({ open, onOpenChange, workflow }: CopyWorkflowToTeamModalProps) {
  ...
  const handleCopy = async () => {
    ...
    const result = await copyWorkflowToTeam(
      workflow,
      selectedTeamId,
      selectedTeam.name,
      user.id,
  ...
  const handleOpenInTeam = () => {
    ...
    navigate(`/workflows/${copiedResult.slug}`);
    onOpenChange(false);
      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Your workflow has been copied to <strong>{copiedResult.teamName}</strong>.
            </DialogDescription>
          </DialogHeader>
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a copy of "{workflow.title}" in a team workspace.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="include-metadata" className="text-sm font-normal">
              Include current tags, category, filename, and description
            </Label>
          </div>
