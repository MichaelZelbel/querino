...
export function CreditsDisplay() {
  ...
  const resetDate = periodEnd
    ? format(new Date(periodEnd), "dd MMM 'at' h:mm a")
    : null;
  ...
  const daysUntilReset = periodEnd
    ? differenceInDays(new Date(periodEnd), new Date())
    : null;
  const showRolloverPreview =
    daysUntilReset !== null && daysUntilReset <= 5 && daysUntilReset >= 0;
  ...
  return (
    <div className="mt-6 pt-6 border-t border-border">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        >
          {Math.round(remainingCredits).toLocaleString()} of{" "}
            ...
            {isEmpty ? (
              ...
              <>
                ...
                {resetDate ? (
                  <>
                    {" "}
                    until reset on <strong>{resetDate}</strong>
                  </>
                ) : null}
                .
              </>
      ...
      {showRolloverPreview && projectedRollover > 0 && !isEmpty && (
        <div className="mt-3 flex items-center gap-2 text-sm text-primary bg-primary/10 rounded-md px-3 py-2">
          ...
          <span>
            <strong>{projectedRollover.toLocaleString()}</strong> credits will
      ...
      <div className="mt-4 space-y-2">
        ...
        {resetDate && (
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Check className="h-4 w-4 text-primary" />
            <span>
              {effectivePlanCredits.toLocaleString()} credits reset on{" "}
              {resetDate}
            </span>
          </div>
        )}
