...
interface CopyArtifactToTeamModalProps<S extends { id: string; title: string }> {
  ...
  source: S;
  /** Human label, e.g. "prompt", "skill" */
  label: string;
  /** Detail-route prefix for the team copy, e.g. "/prompts" */
...
export function CopyArtifactToTeamModal<S extends { id: string; title: string }>({
  ...
  copyToTeam,
  copying,
}: CopyArtifactToTeamModalProps<S>) {
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a copy of "{source.title}" in a team workspace.
          </DialogDescription>
        </DialogHeader>
