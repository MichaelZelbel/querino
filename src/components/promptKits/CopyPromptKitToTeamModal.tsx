...
export function CopyPromptKitToTeamModal({ open, onOpenChange, promptKit }: CopyPromptKitToTeamModalProps) {
  ...
  const handleCopy = async () => {
    ...
    const result = await copyKitToTeam(promptKit, selectedTeamId, selectedTeam.name, user.id, { includeMetadata });
    if (result) {
      setCopiedResult({ slug: result.slug, teamName: result.teamName, teamId: selectedTeamId });
  ...
  const handleOpenInTeam = () => {
    ...
    navigate(`/prompt-kits/${copiedResult.slug}`);
    onOpenChange(false);
      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Your prompt kit has been copied to <strong>{copiedResult.teamName}</strong>.
            </DialogDescription>
          </DialogHeader>
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Create a copy of "{promptKit.title}" in a team workspace.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleCopy} disabled={copying || !selectedTeamId}>
