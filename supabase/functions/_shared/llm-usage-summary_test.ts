...
Deno.test(
  "summarizeUsage: premium_gift counts as paying, admin never does",
  () => {
    const { call_sites } = summarizeUsage(
      [
...
Deno.test(
  "summarizeUsage: a call with no user is machine traffic, not free",
  () => {
    const { call_sites } = summarizeUsage(
...
Deno.test(
  "summarizeUsage: a ledger row written before config_source existed says so",
  () => {
    const { call_sites } = summarizeUsage(
      [
...
Deno.test(
  "summarizeUsage: a configured call site nobody has used still appears",
  () => {
    const { call_sites } = summarizeUsage(
...
Deno.test(
  "summarizeUsage: the most expensive call site is first, dormant ones last",
  () => {
    const { call_sites } = summarizeUsage(
      [
...
Deno.test(
  "freeShareOfTokens: an empty ledger is zero, never a NaN on screen",
  () => {
...
Deno.test(
  "freeShareOfTokens: the share is of every token the app spent",
  () => {
...
Deno.test(
  "normalizeUsageRow: a bigint arrives from Postgres as a string, not a number",
  () => {
...
Deno.test(
  "normalizeUsageRow: a missing number is zero and a missing call site is named",
  () => {
...
Deno.test(
  "summarizeUsage: a name the config table does not carry is flagged, not mixed in",
  () => {
