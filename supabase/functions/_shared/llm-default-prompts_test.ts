...
import { SYSTEM_PROMPT as CANVAS_AI_PROMPT, modeInstructions } from "./prompts/canvas-ai.ts";
...
Deno.test("every default system prompt belongs to a registry call site", () => {
  ...
  const orphans = Object.keys(DEFAULT_SYSTEM_PROMPTS).filter((site) => !known.has(site));
...
Deno.test("a default only uses placeholders the registry declares", () => {
  ...
  for (const meta of CALL_SITES) {
    ...
    for (const name of used) {
      if (!meta.placeholders.includes(name)) wrong.push(`${meta.call_site}: {{${name}}}`);
    }
  }
  assertEquals(wrong, []);
});

Deno.test("every placeholder the registry declares is used by its default", () => {
  const unused: string[] = [];
  for (const meta of CALL_SITES) {
    const text = DEFAULT_SYSTEM_PROMPTS[meta.call_site] ?? "";
    for (const name of meta.placeholders) {
      if (!text.includes(`{{${name}}}`)) unused.push(`${meta.call_site}: {{${name}}}`);
    }
  }
  assertEquals(unused, []);
});

Deno.test("getDefaultSystemPrompt returns the text for a known call site", () => {
  assertEquals(
...
Deno.test("getDefaultSystemPrompt returns null for a call site it does not know", () => {
...
Deno.test("isDefaultSystemPrompt: the untouched default is the default", () => {
  assert(isDefaultSystemPrompt("prompt-wizard", DEFAULT_SYSTEM_PROMPTS["prompt-wizard"]));
});

Deno.test("isDefaultSystemPrompt: surrounding whitespace does not make it custom", () => {
  assert(
    isDefaultSystemPrompt("prompt-wizard", `\n  ${DEFAULT_SYSTEM_PROMPTS["prompt-wizard"]}  \n`),
  );
});

Deno.test("isDefaultSystemPrompt: one edited character makes it custom", () => {
  assertEquals(
    isDefaultSystemPrompt("prompt-wizard", `${DEFAULT_SYSTEM_PROMPTS["prompt-wizard"]}.`),
    false,
  );
});

Deno.test("isDefaultSystemPrompt: nothing matches a call site with no default", () => {
...
Deno.test("normalizeSystemPrompt: saving the prefilled default stores nothing", () => {
  assertEquals(normalizeSystemPrompt("prompt-wizard", DEFAULT_SYSTEM_PROMPTS["prompt-wizard"]), null);
});

Deno.test("normalizeSystemPrompt: an empty box still means the code default", () => {
...
Deno.test("normalizeSystemPrompt: an unknown call site keeps whatever it is given", () => {
...
Deno.test("canvas-ai: the template interpolates to the prompt the old builder produced", () => {
  const vars = {
    mode: "collab_edit",
    artifactType: "skill",
...
Deno.test("a coach default is the coach text plus the shared suffix", () => {
  assertEquals(
    ...
    buildSystemPrompt({ feature: "skill-coach", artifactName: "skill", systemPrompt: SKILL_COACH_PROMPT }),
  );
  assert(DEFAULT_SYSTEM_PROMPTS["skill-coach"].includes("ALWAYS reply by calling the `respond` tool"));
});

Deno.test("each coach's suffix names its own artifact", () => {
  assert(DEFAULT_SYSTEM_PROMPTS["prompt-kit-coach"].includes("the full current prompt kit text"));
  assert(DEFAULT_SYSTEM_PROMPTS["workflow-coach"].includes("the full current workflow text"));
});
