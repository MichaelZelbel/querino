...
const sitemapPaths = await fetch(SITEMAP)
  .then((r) => r.text())
  .then((xml) =>
    [...xml.matchAll(/<loc>https:\/\/querino\.ai([^<]*)<\/loc>/g)].map(
      (m) => m[1] || "/",
    ),
  )
  .catch(() => []);
...
const { routes } = JSON.parse(
  readFileSync(resolve(here, "routes.json"), "utf8"),
);
const slug = (u) =>
  u === "/" ? "root" : u.replace(/^\//, "").replace(/[^\w.-]+/g, "_");
...
writeFileSync(
  resolve(here, "screenshot-urls.json"),
  JSON.stringify(entries, null, 2) + "\n",
);

const counts = entries.reduce(
  (a, e) => ((a[e.data] = (a[e.data] ?? 0) + 1), a),
  {},
);
...
if (unmapped.length)
  console.log(`UNMAPPED: ${unmapped.map((e) => e.route).join(", ")}`);
